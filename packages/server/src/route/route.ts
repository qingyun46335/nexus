  ...
  implements MethodBuilder<E, S, G>
{
   ...
   * 但利用这种假装已注册（实际延迟注册到运行时），也算完成我的大部分设想，也就是解决了Hono 原生类型在跨路由使用c.get/set想要保证类型安全的不可能
   * 但使用时依旧要注意写法：
  ...
  setRoute<E0 extends Env, S0, G0>(
    ...
    this.groupRouteFn.push((app) => {
      r.setFullPath(
        `${this.fullPath != "/" ? (this.fullPath ?? "") : ""}${r.getPrefix()}`,
      );
  ...
  setMethod(fn: (app: Hono<E>) => unknown): EnhancedRoute<E, S, G> {
    this.methodFns.push((app: Hono<E>) => {
      ...
      const after = this.hono.routes;
      const newRoutes = after.slice(before); // 新增的就是刚注册的

      // newRoutes[0] 长这样：
      // { method: "GET", path: "/users", handler: fn }

      return {
        method: newRoutes[0].method,
        path: newRoutes[0].path,
        fullPrefix: this.fullPath,
      };
    });
    return this;
  }

  private methodInfors: methodInfor[] = [];

  protected getMethodInfors(): methodInfor[] {
    return this.methodInfors;
  }

  methodRegister(app: Hono<E>): Result<null> {
    for (const fn of this.methodFns) {
      const methodInfor = fn(app);
      this.methodInfors.push(methodInfor);
    }
    return Ok(null);
  }

  method(app: Hono<E>): Result<null> {
